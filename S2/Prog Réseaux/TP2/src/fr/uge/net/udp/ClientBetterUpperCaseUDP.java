...
public class ClientBetterUpperCaseUDP {
	...
	public static Optional<String> decodeMessage(ByteBuffer buffer) {
		...
		var sizeCharset =  buffer.getInt(); //Taille du Charset utilisé
		var initialLimit = buffer.limit(); // Limit initial du buff
		var limit = buffer.position() + sizeCharset; // Zone de travail correspondant à l'encodage du charset
		if(initialLimit < limit){ // si capacité initial inférieur à capacité supérieur
			return Optional.empty();
		}
		buffer.limit(limit);
