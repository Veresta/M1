...
void removeRing2(cv::Mat &image, const float minRadius, const float maxRadius, const float centerRadius)
{
// image center
    float cx = image.cols/2.0;
    float cy = image.rows/2.0;
...
// try all the pixels
    for(int i=0; i<image.rows; ++i)
        for(int j=0; j<image.cols; ++j){
            float radius = pow((float)i-cy,2) + pow((float)j-cx,2);
            if((i <= cy-centerRadius) && (j <= cx-centerRadius) && (i <= cy+centerRadius) && (j <= cx+centerRadius)){
...
cv::Mat sobelXFourierKernel(const int width, const int height)
{
    ...
    float cx = kernel.cols/2.0;
    float cy = kernel.rows/2.0;
    kernel.at<float>(cx + 1 ,cy) = 2.;
    kernel.at<float>(cx - 1 ,cy) = -2.;
    kernel.at<float>(cx + 1,cy + 1) = 1.;
    kernel.at<float>(cx - 1,cy + 1) = -1.;
    kernel.at<float>(cx + 1,cy - 1) = 1.;
    kernel.at<float>(cx - 1,cy - 1) = -1.;
    return kernel;
...
cv::Mat laplacienFourierKernel(const int width, const int height){
    cv::Mat kernel(height, width, CV_32F, cv::Scalar(0));
    float cx = kernel.cols/2.0;
    float cy = kernel.rows/2.0;
    kernel.at<float>(cx,cy) = -4.;
    kernel.at<float>(cx,cy + 1) = 1.;
    kernel.at<float>(cx,cy - 1) = 1.;
    kernel.at<float>(cx + 1,cy) = 1.;
    kernel.at<float>(cx - 1,cy) = 1.;
    return kernel;
}

cv::Mat q5FourierKernel(const int width, const int height){
    cv::Mat kernel(height, width, CV_32F, cv::Scalar(0));
    float cx = kernel.cols/2.0;
    float cy = kernel.rows/2.0;
    kernel.at<float>(cx,cy) = -5.;
    kernel.at<float>(cx,cy + 1) = -1.;
    kernel.at<float>(cx,cy - 1) = -1.;
    kernel.at<float>(cx + 1,cy) = -1.;
    kernel.at<float>(cx - 1,cy) = -1.;
